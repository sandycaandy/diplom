...
<body>
  <section class="category-product">
    ...
    <div class="container">
    <h1 class="section__title product__list-title">Пледы</h1>
    <div class="product__wrapper-block">
      <div class="block-category">
        ...
        <form class="filter-form" method="GET" action="search_filter-plaids.php">
          ...
          <ul class="checkbox-brand">
            ...
            $result = mysql_query("SELECT * FROM category WHERE type='plaid'", $link);
...
<li><input ' . $checked_brand . ' type="checkbox"name="brand[]" class="checkbox__input" value="' . $row["id"] . '" id="checkbrend' . $row["id"] . '" /><label for="checkbrend' . $row["id"] . '">' . $row["brand"] . '</label></li>
  ...
  ';
          ...
          <ul class="checkbox-type">
            ...
            $result = mysql_query("SELECT * FROM type_cloth WHERE type_cloth='plaid'", $link);
...
<li><input ' . $checked_name_face . ' type="checkbox"name="name_face[]" class="checkbox__input" value="' . $row["id"] . '" id="checktype' . $row["id"] . '" /><label for="checktype' . $row["id"] . '">' . $row["name_face"] . '</label></li>
  ...
  ';
          ...
          <ul class="checkbox-type">
            ...
            $result = mysql_query("SELECT * FROM type_size WHERE type_size='plaid'", $link);
...
<li><input ' . $checked_name_size . ' type="checkbox"name="name_size[]" class="checkbox__input" value="' . $row["id"] . '" id="checktype' . $row["id"] . '" /><label for="checktype' . $row["id"] . '">' . $row["name_size"] . '</label></li>
  ...
  ';
      ...
      <ul class="product__list">
        ...
        $result = mysql_query("SELECT * FROM table_products WHERE type_tovara='plaids' AND  visible='1' $query_brand $query_face $query_size $query_price  ORDER BY products_id DESC", $link);
        ...
        if (mysql_num_rows($result) > 0) {
          ...
          do {
            ...
            <span class="type">' . $row["name_face"] . '</span>
...
';
          ...
          echo '<h3 class="product__nt">Выбранной категории товаров не существует!</3>';
        }
